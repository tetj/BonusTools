...
namespace SteamCommon
{
    public static class Steam
    {
        ...
        public static async Task<string> GetGameSteamIdAsync(Game game, string name, bool useLinksDetection = false)
        {
            string id = "";
            ...
            {
                id =  GetSteamIdFromLinks(game);
            }

            if (String.IsNullOrEmpty(id))
            {
                var searcher = new SteamGameSearcher();                
                var result = await searcher.FindGameByNameAsync(name);

                if (result.AppId.HasValue)
                {
                    Debug.WriteLine($"Found game: {result.GameName} (AppID: {result.AppId})");
                    id = result.AppId.Value.ToString();
                }
                else
                {
                    Debug.WriteLine("Game not found");
                }
            }
